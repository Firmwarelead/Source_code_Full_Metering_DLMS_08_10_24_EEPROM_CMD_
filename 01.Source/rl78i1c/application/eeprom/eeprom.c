...
/* Wrapper */
#include "wrp_app_iic.h"       /* Wrapper/User IIC */
...
uint8_t EPR_Read(uint32_t addr, uint8_t* buf, uint16_t size)
{
    ...
    /* Delay after stop operation: 13us
...
uint8_t EPR_Write(uint32_t addr, uint8_t* buf, uint16_t size)
{
    ...
    while (size > 0)
    {
		...
		/* Wait for write cycle timing */
#if (defined(EEPROM_ACK_BIT_AVAILABLE))
        /* Poll for writing status */
        WRP_IIC_PollingWriteCycleStatus(device_addr, 100, (EPR_DEVICE_WRITE_CYCLE_TIME+99)/100);
#else
        /* Delay after write cycle + stop operation */
        MCU_Delay((EPR_DEVICE_WRITE_CYCLE_TIME+1)); //clkwek
#endif
...
/**************below function add by mrityunjay*********//////////
// Function to erase data from EEPROM
void fill_with_0x00(uint8_t *data, uint16_t size)
{
	uint16_t i ;
    for (i = 0; i < size; ++i) {
        data[i] = 0x00;
...
{
    uint8_t eraseData[EPR_DEVICE_PAGESIZE];
    uint32_t remaining = size;
    uint32_t currentAddress = address;
    uint32_t currentChunkSize;
 
    // Fill the erase data buffer with 0xFF
    //fill_with_0x00(eraseData, EPR_DEVICE_PAGESIZE);
     memset(eraseData, 0, sizeof(eraseData));  // Clear the entire buffer
    MCU_Delay(10000U);
 
    // Write erase data to the EEPROM in chunks
    while (remaining > 0U) {
        currentChunkSize = (remaining > EPR_DEVICE_PAGESIZE) ? EPR_DEVICE_PAGESIZE : remaining;
        EPR_Write(currentAddress, eraseData, currentChunkSize);
