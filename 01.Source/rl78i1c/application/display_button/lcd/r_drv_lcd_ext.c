...
{
	if (ram_addr != (LCD_RAM_START_ADDRESS + 18))
	{
	    *((uint8_t __far *)ram_addr++) |= disp_value & 0xFF;
	    //*((uint8_t __far *)ram_addr++) |= (disp_value >> 4) & 0x0F;
	}
	else
	{
	    *((uint8_t __far *)(LCD_RAM_START_ADDRESS + 18)) |= disp_value & 0xFF;
	    //*((uint8_t __far *)(LCD_RAM_START_ADDRESS + 4)) |= (disp_value >> 4) & 0x0F;
	}
}
