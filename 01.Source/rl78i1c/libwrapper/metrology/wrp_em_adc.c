...
#ifdef __DEBUG
#define EM_MAX_SAMPLE_BUFFER        (390)
...
/* Not related to EM: debug call back entry to attach debug function: trace code,... */
/* Function to near for atomic set */
void (NEAR_FUNC * g_wrp_adc_basic_signal_ready_cb_and_return)(void) = NULL;
void (NEAR_FUNC * g_wrp_adc_signal_ready_cb_and_return)(void) = NULL;
/******************************************************************************
...
void EM_ADC_InterruptCallback(void)
{
    ...
    /* Call back and return for capturing processed basic signals on other application */
    {
        if (g_wrp_adc_basic_signal_ready_cb_and_return != NULL) 
        {
            g_wrp_adc_basic_signal_ready_cb_and_return();
            return;
        }
    }
    ...
    /* Call back and return for capturing processed full signal on other application */
    {
        if (g_wrp_adc_signal_ready_cb_and_return != NULL) 
        {
            g_wrp_adc_signal_ready_cb_and_return();
            return;
        }
    }
